...
class PatchManager {
	...
	onSquare(event) {
		var id = event.currentTarget.id
		debug('onSquare', id, event, gameui.isCurrentPlayerActive(), gameui.isPracticeMode(), gameui.curstate, gameui.clientStateArgs)
		event.preventDefault()
		if (!id) return
		...
		if (gameui.curstate === 'client_PlaceAnimal') {
			...
			if (!gameui.isActiveSlot(correspondingAnmlSquare)) return
			...
			debug('this.selectedNode', this.selectedNode)
...
define([
	...
	return declare('bgagame.newyorkzoo', bgagame.sharedparent, {
		...
		onEnteringState_chooseFence(args) {
			if (args['bonusBreeding']) {
				if (this.isCurrentPlayerActive()) {
					this.setDescriptionOnMyTurn(_('Bonus breeding : ${you} can choose an enclosure that has not already breed this turn'))
				} else {
					this.setDescriptionOnOthersTurn(_('Bonus breeding : ${actplayer} can choose an enclosure that has not already breed this turn'))
				}
			}
		},
		...
		onUpdateActionButtons_playerTurn: function (args) {
			...
			if (this.playerCount == 1) {
				this.setDescriptionOnMyTurn(_('You must move the elephant of the amount of a range token to choose an action'))
		...
		onUpdateActionButtons_populateNewFence: function (args) {
			...
			this.addDoneButton()
			dojo.addClass('button_done', 'disabled')
		...
		onUpdateActionButtons_commonClientPickPatch: function (args) {
			...
			if (controls) {
				gameui.addImageActionButton('rotate_control_b', this.createDiv('rotate-image control-image'), (event) => this.pm.onClickPatchControl(event))
				gameui.addImageActionButton('rotateR_control_b', this.createDiv('rotate-image control-image flip-v'), (event) =>
					this.pm.onClickPatchControl(event)
				)
				gameui.addImageActionButton('flip_control_b', this.createDiv('mirror-image control-image'), (event) => this.pm.onClickPatchControl(event))
		...
		onUpdateActionButtons_commonPlaceAnimal: function (args) {
			...
			for (const anml of choosableAnimals) {
				...
				gameui.addImageActionButton(
					...
					pickcolor,
					_('If you take this animal, you’ll get only one animal instead of the two from your acquisition zone (the blue ones)')
				)
			}
		...
		restoreSquareName: function (squareCompactId) {
			...
			return `square_${strSquareCompactId.charAt(0)}_${strSquareCompactId.charAt(1)}_${strSquareCompactId.slice(2)}`
		},
		...
		/* called to validate the enclosure placing */
		onDone: function () {
			$('overall-content').classList.remove('placingFence')
		...
		getSoloTokenChoice(zoneDiv) {
			...
			if (zoneDiv && dojo.query(`#${zoneDiv.id} .solo-token:not(.hidden)`).length > 1) {
				needed = this.queryFirst(`#${zoneDiv.id} .solo-token:not(.hidden):not(.soloTokenFree)`).dataset.soloTokenId
			}
			return needed
		},
