...
define(['dojo', 'dojo/_base/declare', 'ebg/core/gamegui'], function (dojo, declare) {
    return declare('bgagame.sharedparent', ebg.core.gamegui, {
        constructor: function () {
            debug('sharedparent constructor');
            this.defaultAnimationDuration = 500;
        },
        setup: function (gamedatas) {
            this.inherited(arguments);
            debug('Starting game setup parent');
            ...
            try {
                ...
                if (!this.isSpectator) this.player_no = gamedatas.players[this.player_id].no;
                else this.player_no = gamedatas.players[this.first_player_id].no;
                ...
                this.inSetup = false;
                debug('Ending game setup parent');
            }
        },
        setupPlayer: function (player_id, playerInfo) {
            // does nothing here, override
            debug('player info ' + player_id, playerInfo);
        },
        ...
        onEnteringState: function (stateName, args) {
            ...
            debug('onEnteringState: ' + stateName, args, this.debugStateInfo());
            if (args && args.args) {
                this.gamedatas.gamestate.args = this.processStateArgs(stateName, args.args);
                args = this.gamedatas.gamestate.args;
                debug('processStateArgs: ', this.gamedatas.gamestate.args);
            }
        ...
        onUpdateActionButtons: function (stateName, args) {
            if (this.curstate != stateName) {
                ...
                //debug("skip update "+this.gamedatas.gamestate.name,this.curstate,stateName);
            ...
            if (this.isCurrentPlayerActive()) {
                debug('onUpdateActionButtons: ' + stateName, args, this.debugStateInfo());
                // Call appropriate method
        ...
        addActiveSlot: function (node, handler) {
            ...
            if (typeof handler == 'string') handler = dojo.hitch(this, handler);
            var superhandler = (event) => {
                if (handler(event)) {
        ...
        attachToNewParentNoDestroy: function (mobile_in, new_parent_in, relation, place_position) {
            //debug("attaching ",mobile,new_parent,relation);
        ...
        ajaxActionErrorCallback: function (action, args, message) {
            debug('restoring server state, error: ' + message);
        ...
        ajaxClientStateAction: function (action) {
            ...
            debug('ajaxClientStateAction ', this.clientStateArgs, action);
        ...
        cancelLocalStateEffects: function () {
            ...
            if (this.restoreList) {
                ...
                for (var i = 0; i < restoreList.length; i++) {
                    ...
                    if (tokenInfo !== undefined) this.placeTokenWithTips(token, tokenInfo);
                    else if ($(token)) {
                        dojo.destroy(token);
                    }
                }
        ...
        onClickSanity: function (event, checkActive) {
            ...
            debug('on slot ' + id);
            if (!id) return null;
            if (checkActive && !id.startsWith('button_') && !this.checkActiveSlot(id)) {
                return null;
            }
        ...
        isActiveSlot: function (id) {
            ...
            if (dojo.hasClass(id, 'hidden_active_slot')) {
                return true;
            }
        ...
        childIsActiveSlot: function (id) {
            if (!$(id) || !$(id).firstElementChild) {
                return false;
            }
            const childId = $(id).firstElementChild.id;
            if (dojo.hasClass(childId, 'active_slot')) {
                return true;
            }
            if (dojo.hasClass(childId, 'hidden_active_slot')) {
                return true;
            }
        ...
        divActivePlayer: function (args) {
            ...
            var elt =
                ...
                color_bg +
                '">' +
                __('lang_mainsite', args.actplayer) +
                '</span>';
            return elt;
        },
        ...
        setDescriptionOnAnyTurn: function (text, descriptionType, moreargs) {
            ...
            //debug('in',   this.gamedatas.gamestate.args, moreargs);
            var tpl = dojo.clone(this.gamedatas.gamestate.args);
        ...
        getTokenState: function (tokenO) {
            ...
            if (tokenInfo) return parseInt(tokenInfo.state);
        ...
        getActionLine: function (text) {
            return (
                ...
                g_themeurl +
                "img/layout/help_click.png' alt='action' /> <span class='tooltiptext'>" +
                text +
                '</span>'
            );
        },
        ...
        placeTokenLocal: function (token, place, state, args) {
            ...
            if (state !== null && state !== undefined) tokenInfo.state = state;
        ...
        animEvaporResource: function (animNodeId, inc, div) {
            ...
            for (var i = 0; i < mod; i++) {
                ...
                setTimeout(
                    dojo.hitch(this, function (local) {
                        // debug("apply "+local);
        ...
        placeToken: function (token, tokenInfo, args) {
            try {
                ...
                if (place == 'destroy') {
                    if (tokenNode) {
                        // debug(token + ": " + tokenInfo.type + " -destroy-> " + place + " " + tokenInfo.state);
                ...
                if (tokenNode == null) {
                    ...
                    //debugger;
                    tokenNode = this.createToken(
                        token,
                        tokenInfo,
                        placeInfo.createOn ? placeInfo.createOn : place,
                        placeInfo.onclick
                    );
                    if (tokenNode == null) {
                        return;
                ...
                // debug(token + ": " + tokenInfo.key + " -move-> " + place + " " + tokenInfo.state);
                var state = 0;
                ...
                if (placeInfo.x !== undefined || placeInfo.y !== undefined) {
                    ...
                    if (tokenNode.parentNode.id == place) {
                        // already there
                        //debug(token+" already there at "+place);
                    } else {
                        if (noAnnimation) {
                            if (placeInfo.temp) {
                                dojo.destroy(token);
        ...
        createToken: function (token, tokenInfo, place, connectFunc) {
            ...
            if (place) {
                //debug(token + "- created -> [" + place+"]", tokenDiv, tokenInfo);
                if (!$(place)) {
                ...
                if (connectFunc) {
                    //debug("new connect -> "+connectFunc, tokenNode);
        ...
        updateMyCountersAll: function () {
            if (this.gamedatas.gamestate.args && this.gamedatas.gamestate.args.counters) {
                // debug(this.gamedatas.gamestate.args.counters);
        ...
        doMoveToken: function (token_id, place_id, new_state) {
            ...
            //debug("** notif moved " + token + " -> " + place_id + " (" + new_state + ")");
        ...
        notif_log: function (notif) {
            ...
            if (notif.log) {
                debug('log notif: ' + this.format_string_recursive(notif.log, notif.args), notif.log, notif.args);
            }

            if (notif.args && notif.args.log) {
                debug(
                    'log notif: ' + this.format_string_recursive(notif.args.log, notif.args.args),
                    notif.args.log,
                    notif.args.args
                );
            }
        ...
        notif_counter: function (notif) {
            try {
                ...
                this.updateCountersSafe(this.gamedatas.counters);
                //                debug("** notif counter " + notif.args.counter_name + " -> " + notif.args.counter_value);
