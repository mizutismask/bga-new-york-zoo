...
abstract class EuroGame extends APP_Extended
{
    ...
    protected function getDefaultCounters()
    {
        ...
        foreach ($types as $key => $info) {
            ...
            if (array_key_exists('loc', $info) && $info['loc'] && $counter == 1) {
                if ($info['loc'] == 1) {
                    $this->setCounter($res, "${key}_counter", 0);
                } else  if ($info['loc'] == 2) {
                    foreach ($players_basic as $player_id => $player_info) {
                        $order = $player_info['player_nb'];
                        $this->setCounter($res, "${key}_${order}_counter", 0);
                    }
                }
            }
        }
        ...
        if (array_key_exists($key, $this->token_types)) {
            ...
            if (isset($info['loc'])) {
                if (array_key_exists($attr, $info)) {
                    if ($info[$attr] == 0) { // content not visible
                        return false;
                    }
                    if ($info[$attr] == 1) { // content visible for all
                        return true;
                    }
                    if ($info[$attr] == 2) { //content visible for current player
                        $order = $this->getPlayerPosition($player_id);
                        return endsWith($location, $order);
                    }
                    $this->systemAssertTrue("Invalid value for $attr of $key");
                } else {
                    return true; // no content field - allowed
                }
            } else {
    ...
    protected function isCounterAllowedForLocation($player_id, $location)
    {
        ...
        if (array_key_exists($key, $this->token_types)) {
            ...
            if (array_key_exists('loc', $info) && $info['loc']) {
                $counter = $info[$attr] ?? 0;
                if ($counter == 1) { // counter visible for all
                    return true;
                }
                if ($counter == 2) { // counter visible for current player
                    $order = $this->getPlayerPosition($player_id);
                    return endsWith($location, $order);
                }
            }
     ...
     * @param * $token_arr
     */
    function dbSyncInfo($token_arr, $animate = false)
    {
        ...
        foreach ($token_arr as $info) {
            ...
            $state = $info['state'];
            $notifyArgs = array(
                'token_id' => $token_id, 'place_id' => $place_id, 'place_name' => $place_id,
                'new_state' => $state, 'noa' => !$animate, 'nod' => true
            );
